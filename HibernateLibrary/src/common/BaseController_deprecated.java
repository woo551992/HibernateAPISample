 ...
 * </PRE>
 */
public class BaseController_deprecated {
	...
	public BaseController_deprecated(Session session){
...
//region search
	 ...
	 */
	protected <T> Criteria createSearchCriteria(SearchMode searchMode, Class<T> searchEntity, T entityCriteria){
			...
			if (entity.getIdentifier().getValue() != null) {	// such as user.getUsername()!=null, or account.getId()!=null <- getId() is composite primary keys 
				if (entity.hasSinglePrimaryKey()) {
					...
					for (Field field : entity.getIdentifier().getValue().getClass().getDeclaredFields()) {
						...
						if (value != null) { 									// include fields which is not null
							criteria.add(Restrictions.eq(entity.getIdentifier().getPropertyName() + "." + field.getName(), value));
						}
					}
				} else {
